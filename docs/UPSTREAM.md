...
| --- | --- | --- | --- | --- |
| CPA / Codex 模型检测 | [router-for-me/CLIProxyAPI](https://github.com/router-for-me/CLIProxyAPI) | `c76dfd4e0edabab9000628b1560ab8ab379eadb8`（短：`c76dfd4e`） | 2026-09-06 | `internal/auth/codex/token.go`、`jwt_parser.go`、`cmd/fetch_codex_models/main.go`、`internal/runtime/executor/codex_executor_request.go` |
| sub2api | [Wei-Shaw/sub2api](https://github.com/Wei-Shaw/sub2api) | `ab99d56e9626e6cd731592dae8553c9758a0efa2`（短：`ab99d56e`） | 2026-09-05 | `backend/internal/handler/admin/account_codex_import.go`、`backend/internal/pkg/openai/oauth.go`（`ClientID`） |
...
1. diff 上表对应文件；
2. 改映射算法；
3. 更新 `config/upstream-audit.json`、生成的 `SOURCE_PINS` 与本表（Rust `upstream.rs` 编译时嵌入同一 JSON）；
4. 同步缓存版本号并执行回归测试。
