...
| --- | --- | --- | --- | --- |
| CPA | [router-for-me/CLIProxyAPI](https://github.com/router-for-me/CLIProxyAPI) | `42a00a2a6521b867c27f7ad096d08699db8e6d19`（短：`42a00a2a`） | 2026-07-26 | `internal/auth/codex/token.go`（`CodexTokenStorage`）、`jwt_parser.go` |
| sub2api | [Wei-Shaw/sub2api](https://github.com/Wei-Shaw/sub2api) | `2730c1c43b29be003925b033f3f9e645e726bb8c`（短：`2730c1c4`） | 2026-07-25 | `backend/internal/handler/admin/account_codex_import.go`、`backend/internal/pkg/openai/oauth.go`（`ClientID`） |

页面顶部「已按审计版本源码映射」徽章由 `bridge.js` 的 **`SOURCE_PINS`** 渲染（HTML 含首屏静态回退）。这表示转换规则已针对明确的固定提交进行审计，而不是声称始终跟随上游 HEAD。两仓更新后：

1. diff 上表对应文件；
2. 改映射算法；
3. 只更新 `SOURCE_PINS` 的 `shortSha` / `fullSha` / `date` / `commitUrl`；
4. 同步本文件与缓存版本号。
...
- CLIProxyAPI 的 `internal/auth/codex/token.go` 与 `jwt_parser.go` 在 `411d7d41 → 42a00a2a` 之间逐字节相同，CPA 映射无需改动，仅重新钉版本；
...
- 模式 `sub2api → CPA`：`sub2api-data` / account 对象 → CPA auth；多账号下载 ZIP（每账号一个文件）。
